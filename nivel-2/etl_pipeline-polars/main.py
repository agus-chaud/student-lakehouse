...
def stage_silver(cfg: dict, s3) -> dict:
    ...
    silver_frames = (
        transform_silver.run(pl.from_arrow(pqfile.read_row_group(index)))
...
def stage_gold(cfg: dict, s3) -> dict:
    ...
    with measure("transform_gold", rows=df_silver.height):
        gold_models = transform_gold.run(df_silver, df_zones)
    del df_silver
    with measure("load_gold", rows=sum(m.height for m in gold_models.values())):
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="ETL Pipeline — NYC Taxi Lakehouse (Polars)")
